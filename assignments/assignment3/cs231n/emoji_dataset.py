...
class ClipEmbed:
    def __init__(self, device, download_root=None):
        self.model, self.preprocess = clip.load("ViT-B/32", device=device, download_root=download_root)
...
class TextEmbedder:
    ...
    def load_processed(self, data_path):
        self.loaded = torch.load(data_path, map_location="cpu", weights_only=True)

    def save_processed(self, all_texts, path, device="cpu"):
        assert not os.path.exists(path)
        text_embedder = ClipEmbed(device=device)
